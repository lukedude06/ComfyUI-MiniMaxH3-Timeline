"""VENDORED SNAPSHOT of comfy/ldm/minimax/model.py (ComfyUI v0.34.2).

ComfyUI-MiniMaxH3-Timeline patches the MiniMax H3 forward pass. The math it
builds on -- PackedLayout, the position grids, patchify/pack, the RoPE table,
and the cond-noise-aug constants -- lives here, PINNED, so a ComfyUI update
cannot silently change it under the timeline logic. Only the loaded model's
*neural* submodules (blocks / final_layer / projections) still come from the
running ComfyUI; those are checkpoint-frozen and their call sites are
signature-adapted in nodes.py.

To re-sync after a deliberate ComfyUI bump: diff this file against
comfy/ldm/minimax/model.py at that tag, port real changes, re-test a render.
"""
